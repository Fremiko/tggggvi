...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = (
        "🐶 Добро пожаловать в Мопс-Бота!\n\n"
        "📚 Команды: /команды\n"
        "📖 Подробный гайд: /гайд\n"
        "💰 Экономика: /баланс /казино /банк\n"
        "⚔️ PvP: /дуэль /война\n"
        "🎮 Игры: /words_start /монетка\n"
        "🐾 Питомцы: /питомец\n"
    )
...
HOSHI_EXTRA_PHRASES = [
    "🐱 Хоши: иногда лучший фарм — это отдых.",
    "🐱 Хоши заметила редкий лут рядом.",
    "🐱 Хоши советует прокачивать не только силу, но и удачу.",
    "🐱 Хоши приготовила чай перед рейдом.",
    "🐱 Хоши: сегодня отличный день для побед.",
    "🐱 Хоши нашла секретный бонус.",
    "🐱 Хоши: не забывай про ежедневные задания.",
    "🐱 Хоши: даже маленький фарм приводит к богатству.",
    "🐱 Хоши гладит Мопса-Фармилу за хорошую работу.",
    "🐱 Хоши: легенды начинаются с первого клика.",
]

FARMILA_EXTRA_PHRASES = [
    "🐶 Мопс-Фармила уже копает монеты.",
    "🐶 Мопс-Фармила принес мешок удачи.",
    "🐶 Мопс-Фармила ворчит на курс цен.",
    "🐶 Мопс-Фармила устроил мини-праздник.",
    "🐶 Мопс-Фармила нашёл легендарный предмет.",
    "🐶 Мопс-Фармила предлагает пойти в рейд.",
    "🐶 Мопс-Фармила считает прибыль.",
    "🐶 Мопс-Фармила случайно уронил сундук.",
    "🐶 Мопс-Фармила: главное — фармить с удовольствием.",
    "🐶 Мопс-Фармила танцует после победы."
]

# ================= НОВЫЕ ФУНКЦИИ МАЙ 2026 =================
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lvl = levels.get(uid, 1) if 'levels' in globals() else 1
    ...
    text = (
        ...
        f"💰 Баланс: {bal}\n"
        f"⭐ Уровень: {lvl}\n"
        f"🏆 Достижения: {len(ACHIEVEMENTS.get(uid, []))}\n"
    )
    await update.message.reply_text(text)

async def kazino(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("🎰 Использование: /казино сумма")
        return
    ...
    if random.randint(1, 100) <= 45:
        win = amount * 2
        balances[uid] += amount
        await update.message.reply_text(f"🎉 Победа! +{amount} монет.")
    else:
        balances[uid] -= amount
        await update.message.reply_text(f"💀 Проигрыш! -{amount} монет.")
...
async def help_mega(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📚 Полный гайд доступен в файле guide_full.html"
    )
...
async def pet_feed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    ...
    if PETS[uid]["exp"] >= 50:
        PETS[uid]["level"] += 1
        PETS[uid]["exp"] = 0
        await update.message.reply_text("🎉 Питомец повысил уровень!")
...
async def bank(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🏦 В банке хранится: {BANKS[uid]} монет"
    )

async def bank_put(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)

    try:
        amount = int(context.args[0])
    except:
        await update.message.reply_text("Использование: /вклад сумма")
        return
    ...
    balances[uid] -= amount
    BANKS[uid] = BANKS.get(uid, 0) + amount

    save_data()

    await update.message.reply_text(f"🏦 Вклад пополнен на {amount}")
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, (uid, bal) in enumerate(top, start=1):
        text += f"{i}. {uid} — {bal} монет\n"

    await update.message.reply_text(text)

# ===== ДОП. КОМАНДЫ =====
app.add_handler(CommandHandler("профиль", profile))
app.add_handler(CommandHandler("казино", kazino))
app.add_handler(CommandHandler("монетка", coin))
app.add_handler(CommandHandler("питомец", pet))
app.add_handler(CommandHandler("кормить", pet_feed))
app.add_handler(CommandHandler("банк", bank))
app.add_handler(CommandHandler("вклад", bank_put))
app.add_handler(CommandHandler("топ", leaderboard))
app.add_handler(CommandHandler("помощь", help_mega))
...
def ask_ai(user_id, prompt, personality="default"):
    if user_id not in AI_MEMORY:
        AI_MEMORY[user_id] = []

    system_prompt = {
        "default": "Ты умный Telegram-бот Мопс-Бот.",
        "hoshi": "Ты Хоши — милая аниме-кошка помощница.",
        "farmila": "Ты Фармила — мемный мопс-фармер."
    }.get(personality, "Ты Telegram-бот.")
...
async def ai_command(update, context):
    ...
    if not prompt:
        await update.message.reply_text("Использование: /ии текст")
        return
    ...
    try:
        ...
        await wait_msg.edit_text(f"❌ AI ошибка: {e}")

async def hoshi_ai(update, context):
    user_id = str(update.effective_user.id)
    prompt = " ".join(context.args)

    if not prompt:
        await update.message.reply_text("Использование: /хоши текст")
        return

    answer = ask_ai(user_id, prompt, "hoshi")
    await update.message.reply_text(answer[:4096])

async def farmila_ai(update, context):
    user_id = str(update.effective_user.id)
    prompt = " ".join(context.args)

    if not prompt:
        await update.message.reply_text("Использование: /фармила текст")
        return

    answer = ask_ai(user_id, prompt, "farmila")
    await update.message.reply_text(answer[:4096])

async def art_command(update, context):
    prompt = " ".join(context.args)

    if not prompt:
        await update.message.reply_text("Использование: /арт описание")
        return
...
async def quest_ai(update, context):
    user_id = str(update.effective_user.id)

    prompt = "Придумай интересный RPG квест для Telegram игры."
...
# ===== AI HANDLERS =====
app.add_handler(CommandHandler("ии", ai_command))
app.add_handler(CommandHandler("ai", ai_command))
app.add_handler(CommandHandler("хоши", hoshi_ai))
app.add_handler(CommandHandler("фармила", farmila_ai))
app.add_handler(CommandHandler("арт", art_command))
app.add_handler(CommandHandler("квест", quest_ai))
...
async def bonus(update, context):
    ...
    DAILY_BONUS[user_id] = DAILY_BONUS.get(user_id, 0) + 1

    reward = 500 * DAILY_BONUS[user_id]
...
async def reputation(update, context):
    ...
    await update.message.reply_text(
        f"⭐ Репутация повышена.\n"
        f"Теперь: {REPUTATION[target]}"
    )
...
async def case_open(update, context):
    ...
    INVENTORY.setdefault(user_id, []).append(reward)
...
async def duel_ai(update, context):
    enemy = random.choice([
        "👹 Демон",
        "🤖 AI-бот",
        "🐉 Дракон",
...
async def meme(update, context):
    memes = [
        "🐶 Мопс снова украл всю экономику.",
        "🐱 Хоши опять сломала рынок.",
        "💀 Фармила продал воздух за 5000.",
        "🪙 Игроки фармят 25 часов в сутки."
    ]
...
async def weather_fake(update, context):
...
app.add_handler(CommandHandler("кейс", case_open))
app.add_handler(CommandHandler("битва", duel_ai))
app.add_handler(CommandHandler("мем", meme))
app.add_handler(CommandHandler("погода", weather_fake))
