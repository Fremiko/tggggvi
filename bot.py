...
async def ai_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "делает краткий пересказ и отвечает локальным режимом.\n\n"
...
async def ai_analyze_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not _ai_is_ready():
        await msg.reply_text(
            "Фото получил. Без vision-AI ключа я не могу честно распознать изображение, "
            "но могу решить задачу, если ты пришлёшь её текстом: /solve 2x+5=17.\n"
            "Бот при этом полностью работает без OPENAI_API_KEY."
        )
        return
...
async def ai_photo_auto(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    trigger_words = ("реши", "задача", "анализ", "проанализируй", "что на фото")
